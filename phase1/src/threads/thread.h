...
struct thread
  {
    ...
    struct list lock_list;              /* List of all locks are held by this thread. */

    /* Shared between thread.c and synch.c. */
    struct list_elem elem;              /* List element. */

//+++++++++++++++++++ added (Owned by thread.c) ++++++++++++++++++++++++++++++++=

   int effective_priority;             /* actual Priority. */
   struct lock* waiting_lock;          /* lock that is waiting for .*/
   int64_t sleep_endtick;		         /* The tick after which the thread should awake (if the thread is in sleep) */
   int nice;                           /* nice value /mlfqs.*/
   int recent_cpu;                     /* recent_cpu value /mlfqs .*/
...
void recursive_donation(struct lock* lock, int max_donation);
