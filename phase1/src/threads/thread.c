...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  if(thread_mlfqs)
  {
    ...
    if(timer_ticks() % 100 == 0)
    {
      ...
      int decay = div_fx(mul_in(load_avg, 2),add_in(mul_in(load_avg, 2), 1));
         ...
         e = list_next (e))
      {
        ...
        f->recent_cpu = add_in(mul_fx(decay, f->recent_cpu),f->nice);
      }
    }
...
thread_set_priority (int new_priority) 
{
  ...
  //handle set priority druing donation
  if(!list_empty(&thread_current()->lock_list)){
    struct lock* max_pri_lock= list_entry(list_front(&thread_current()->lock_list)
    ,struct lock,elem);

    if(new_priority > max_pri_lock->semaphore.max_priority){
...
init_thread (struct thread *t, const char *name, int priority)
{
  ...
  if(thread_mlfqs)
  {
    ...
    t -> priority = PRI_MAX - convert_to_intround(div_in(t->recent_cpu,4)) - (t->nice * 2);     ////////////////////////////////
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
/*added function*/
void recursive_donation(struct lock* lock, int donated_priority){
   if(lock == NULL)
      return;
   if(lock->holder->priority < donated_priority){
    lock->holder->priority = donated_priority;
    struct thread *temp_thread;
    temp_thread = list_entry (list_begin (&lock->semaphore.waiters), struct thread, elem);
    lock->semaphore.max_priority=temp_thread->priority;
    list_sort(&lock->holder->lock_list, comp_max_pri, NULL);
    recursive_donation(lock->holder->waiting_lock, donated_priority);
