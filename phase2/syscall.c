...
/*
this function used to check the validation the address which pointed by pointer 
*/
...
{    
    ...
    else if (choose == SYS_EXEC)
    {
        ...
        //store the pid for the created child in EAX Register
...
syscall_handler (struct intr_frame *f )
{
    ...
    //the intr_frame struct provides a snapshot of the processor state during an interrupt or exception
    int syscall_number = 0;
    check_valid_ptr((const void*) f -> esp);
    void *args = f -> esp;
    //Refrenced to the system call
    syscall_number = *( (int *) f -> esp);
    // Move args pointer to arguments to be passed 
    args+=4;
...
void exit (int status)
{
    ...
    if (status == -1)
    {
        ...
        //printf("THIS THREAD SHOULD BE KILLED...\n");
    }
    else
    {
...
exec (const char *cmd_line)
{
    ...
    // make (parant) wait this child until load
