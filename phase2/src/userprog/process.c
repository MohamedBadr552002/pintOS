...
process_wait (tid_t tid)
{
    ...
    if(child -> first_time)
    {
        ...
        {
            // make the current thread (parant) wait this child
...
void get_stack_args(char *file_name, void **esp, char **save_ptr)
{
    ...
    char *token = file_name; /*set token with inital value to the program name*/
    void *stack_pointer = *esp;
    int argc = 0;
    int total_length = 0;

    // allocate dynamic memory to save the arguments addresses
    ...
    while (token != NULL)
    {
        ...
        stack_pointer -= arg_length; // Stored in stack in downward
        /*push the address of each string plus a null pointer sentinel, on the stack, 
        in right-to-left order. These are the elements of argv.*/
        memcpy(stack_pointer, token, arg_length);
        argc++;
        token = strtok_r(NULL, " ", save_ptr);
    }

    // save the last location for parsed parameters
    char *args_pointer = (char *) stack_pointer; 

    /*adding word align*/
    int  word_align = 0;
    //Checking how much align the stack need
    while (total_length % 4 != 0)  
    {
        word_align++;
        total_length++;
    }
    //Perform the alignment Due to the previous checking
    if (word_align != 0)
    {
    ...
    stack_pointer -= sizeof(char **);
    *((char ***) stack_pointer) = first_fetch;  /*save the last location of the stack /
